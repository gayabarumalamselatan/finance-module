...
const EditPurchaseOrder = ({ setIsEditingPurchaseOrder, handleRefresh, index, item, selectedData }) => {
    ...
    useEffect(() => {
        if (selectedData) {
            ...
            LookupService.fetchLookupData(`PURC_FORMPUREQ&filterBy=PR_NUMBER&filterValue=${PO_NUMBER}&operation=EQUAL`, authToken, branchId)
                .then(response => {
            ...
            LookupService.fetchLookupData(`PURC_FORMPUREQD&filterBy=PR_NUMBER&filterValue=${PO_NUMBER}&operation=EQUAL`, authToken, branchId)
                .then(response => {
                    const fetchedItems = response.data || [];

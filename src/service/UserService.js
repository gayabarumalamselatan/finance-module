...
import { AUTH_SERVICE_LIST_USER, AUTH_SERVICE_LIST_USER_DETAIL } from '../config/ConfigUrl';

const UserService = {
  fetchUserData: async (idUser, headers) => {
    try {
      const response = await axios.get(`${AUTH_SERVICE_LIST_USER_DETAIL}?id=${idUser}`, { 
        headers: {

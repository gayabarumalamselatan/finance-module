...
const LookupParamService = {
    ...
    fetchLookupDataView: async (lookupTable, headers, branchId) => {
        try {
            const response = await axios.get(
    ...
    fetchLookupDataLookup: async (lookupTable, headers, branchId) => {
        try {
            const response = await axios.get(
                `${FORM_SERVICE_LOAD_DATA}?t=${lookupTable}&branchId=${branchId}&page=1&size=500&lookup=YES`,
                {
                    headers: {
                        Authorization: `Bearer ${headers}`, // Tambahkan token otorisasi di sini
                    },
...
export default LookupParamService;
